...
static const struct board_uart uarts[2] =
{
	[0] =
	{
		.uart = UART0,
...
const struct board_uart *const board_uarts = uarts;

IRQn_Type board_swi_lookup(unsigned int swi)
{
	assert(swi < BOARD_SWI_NUM);
	return swi + RTC_IRQ_IRQn + 1;
}
...
static void board_uart_init(void)
{
    ...
    uint32_t baud_rate_div = ((BOARD_CLOCK_PERI_HZ * 8) / BOARD_CONSOLE_BAUD_RATE);
