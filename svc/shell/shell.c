...
static int shell_output(struct microrl* mrl, const char* str)
{
	ssize_t written;
	ssize_t amount = 0;
	ssize_t count = strlen(str);

	int fd = (int)file_get_platform(stdout);

	while (amount < count) {
		written = write(fd, str + amount, count - amount);
		if (written < 0)
			return written;
		amount += written;
	}
...
static void shell_task(void *context)
{
	...
	char buffer[MICRORL_CFG_CMDLINE_LEN];
	int fd = (int)file_get_platform(stdin);

	puts(SHELL_MOD);
	buffer[0] = '\n';
	microrl_processing_input(&shell->microrl, buffer, 1);
	...
	while (shell->run) {
		...
		microrl_processing_input(&shell->microrl, buffer, amount);
	}
...
static __constructor_priority(SERVICES_SHELL_PRIORITY) void shell_ini(void)
{
	/* Allocate the shell */
...
static __destructor_priority(SERVICES_SHELL_PRIORITY) void shell_fini(void)
{
	assert(shell != 0);
