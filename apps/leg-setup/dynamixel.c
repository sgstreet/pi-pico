...
int dynamixel_transceive_packet(struct dynamixel_port *port, struct dynamixel_instr_packet *tx_packet, struct dynamixel_status_packet *rx_packet, unsigned int timeout)
{
	...
	int status = dynamixel_tx_packet(port, (struct dynamixel_packet *)tx_packet);
	if (status < 0)
		return status;

	/* Receive the status packet */
	status = dynamixel_rx_packet(port, (struct dynamixel_packet *)rx_packet, timeout);
	if (status < 0)
		return status;
...
int dynamixel_ping(struct dynamixel_port *port, uint8_t id, struct dynamixel_info *info, size_t count)
{
	...
	request.header.length = dynamixel_sizeof(struct dynamixel_ping_request);
	request.instr.instruction = DYNAMIXEL_PING_INSTR;

	/* Initialize response */
	struct dynamixel_ping_response response;
	response.header.length = dynamixel_sizeof(struct dynamixel_ping_response);

	/* Send the instruction packet */
	int status = dynamixel_tx_packet(port, (struct dynamixel_packet *)&request);
	if (status < 0)
		return status;
	...
	for (i = 0; i < count; ++i) {
		...
		/* Receive a response */
		status = dynamixel_rx_packet(port, (struct dynamixel_packet *)&response, DYNAMIXEL_TIMEOUT_MSEC);
		if (status < 0) {
			if (status == -ETIMEDOUT)
				break;
			return status;
		...
		info[i].id = response.header.packet_id;
		info[i].model = response.model;
		info[i].version = response.version;
	}
...
int dynamixel_read(struct dynamixel_port *port, uint8_t id, uint16_t addr, void *buffer, size_t count)
{
	...
	/* Do it */
	int status = dynamixel_transceive_packet(port, (struct dynamixel_instr_packet *)&request, (struct dynamixel_status_packet *)response, DYNAMIXEL_TIMEOUT_MSEC);
	if (status < 0)
		return status;
...
int dynamixel_port_ini(struct dynamixel_port *port, unsigned int channel, unsigned int baud_rate, unsigned int rx_timeout)
{
	...
	int status = half_duplex_configure(port->hd, baud_rate, rx_timeout);
	if (status < 0)
		return status;
...
struct dynamixel_port *dynamixel_port_create(unsigned int channel, unsigned int baud_rate, unsigned int rx_timeout)
{
	...
	/* Initialize */
	int status = dynamixel_port_ini(port, channel, baud_rate, rx_timeout);
	if (status < 0)
		free(port);
