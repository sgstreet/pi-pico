...
bench_time_t bench_timing_counter_get(void)
{
	...
	current.low = SysTick->VAL;
	current.high = TIMER->TIMERAWL;
	return current.val;
}

bench_time_t bench_timing_cycles_get(bench_time_t *time_start, bench_time_t *time_end)
{
	bench_time_t start_cycles = ((*time_start >> 32) * 125ULL) + (((*time_start & 0xffffffff) + 1)% 125);
	bench_time_t end_cycles = ((*time_end >> 32) * 125ULL) + (((*time_end & 0xfffffffff) + 1) % 125);
