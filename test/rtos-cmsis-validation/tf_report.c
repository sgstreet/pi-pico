...
bool tr_File_Close (void) {
  ...
  PRINT(("</report>%s", TF_EOL));
#else
  PRINT(("\nTest Summary: %lu Tests, %lu Executed, %lu Passed, %lu Failed, %lu Warnings.%s",
         test_report.tests,
...
static __unused int printf_lim (const char *fmt, ...) {
  va_list arg;
  int len;
